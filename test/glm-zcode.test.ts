...
describe("off-peak streamSimple wire routing", () => {
  afterEach(() => {
  ...
  const JWT = "wire-jwt";
  const CAPTURE = "/tmp/ulw-pr9/wire-capture.json";
  ...
  const NEXT_DAY_WINDOW = new Date("2026-09-10T18:00:00Z");
  ...
  const markerModel = () => ({
    ...
    headers: { "X-ZCode-Route": "off-peak" },
    id: "glm-5.3-flash",
    name: "GLM-5.3-Flash",
    reasoning: true,
    input: ["text"],
  ...
  test("fresh ticket in-window: wire goes to the off-peak gateway with JWT auth and no signature", async () => {
    ...
    try {
      ...
      vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
        ...
        if (url.endsWith("/off-peak/ticket")) {
          ticketNo += 1;
          return json({ ticket_id: `t-${ticketNo}`, state: "ready" });
        }
        wire.push({ url, method: init?.method, headers: Object.fromEntries(new Headers(init?.headers).entries()), body: init?.body });
        return sseResponse();
      }));
      const stream = config.streamSimple!(markerModel(), wireContext, { apiKey: KEY, headers: { Authorization: `Bearer ${KEY}` } } as never);
      const message = await stream.result();
      expect(message.content).toEqual([{ type: "text", text: "OK" }]);
      const model = wire.find((w) => (w as { url: string }).url.includes("/v1/messages")) as { url: string; headers: Record<string, string> };
  ...
  test("ticket failure and out-of-window: wire falls back to the signed ultra gateway", async () => {
    for (const [label, clock, takeStatus] of [
      ...
      try {
        ...
        expect(message.content).toEqual([{ type: "text", text: "OK" }]);
        const model = wire.find((w) => (w as { url: string }).url.includes("/v1/messages")) as { url: string; headers: Record<string, string> };
        expect(model, `${label}: model wire entry`).toBeDefined();
        ...
        expect(model.headers["x-client-sig"]).toBe("sig");
      } finally {
        setOffPeakClockForTests(undefined);
        vi.unstubAllGlobals();
      }
  ...
  test("cached tickets are revalidated across windows and after the freshness TTL", async () => {
    setOffPeakClockForTests(IN_WINDOW);
    try {
      const { config } = captureProvider();
      config.oauth!.getApiKey({ access: KEY, refresh: "r", expires: 1, zcodeJwtToken: JWT });
      ...
      vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
        ...
        if (url.endsWith("/off-peak/ticket")) {
          takes.push(Date.now());
          return json({ ticket_id: `t-${takes.length}`, state: "ready" });
        }
        if (url.includes("/v1/messages")) wire.push(url);
        return sseResponse();
      }));
      const run = () => config.streamSimple!(markerModel(), wireContext, { apiKey: KEY, headers: { Authorization: `Bearer ${KEY}` } } as never).result();

      await run(); // take #1
      expect(takes).toHaveLength(1);

      setOffPeakClockForTests(new Date(IN_WINDOW.getTime() + 11 * 60_000)); // TTL expiry, same window
      await run();
      expect(takes).toHaveLength(2); // revalidated

      setOffPeakClockForTests(NEXT_DAY_WINDOW); // next day's window
      await run();
      expect(takes).toHaveLength(3); // revalidated across windows

      expect(wire.every((url) => url === OFFPEAK_MESSAGES)).toBe(true);
    } finally {
      setOffPeakClockForTests(undefined);
      vi.unstubAllGlobals();
    }
