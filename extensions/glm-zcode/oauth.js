...
function data(payload) {
    if (!isRecord(payload))
        throw new Error("GLM ZCode response was not an object");
    return isRecord(payload.data) ? payload.data : payload;
}
async function request(url, options, signal, label) {
    if (signal?.aborted)
        throw new Error(`GLM ZCode ${label} request cancelled`);
    ...
    if (!response.ok)
        throw new Error(`GLM ZCode ${label} request failed: ${response.status}`);
    try {
...
async function post(url, body, signal, label, token) {
    return request(url, {
        method: "POST",
        headers: {
...
async function get(url, signal, label, token) {
    return request(url, { headers: { Accept: "application/json", Authorization: `Bearer ${token}` } }, signal, label);
}
function callbackCode(value, state) {
    const input = value.trim();
    if (!input)
        throw new Error("GLM ZCode authorization callback URL is required");
    let callback;
    try {
    ...
    if (states[0] !== state)
        throw new Error("GLM ZCode callback state did not match");
    return codes[0];
}
async function provision(upstreamToken, signal) {
...
export async function loginGlmZcode(callbacks) {
...
export async function refreshGlmZcode(credentials) {
